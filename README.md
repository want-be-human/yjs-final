...
当前已在华为云华北-北京四创建 CCE 集群 `yjs-final-cluster`，并创建 SWR 组织 `yjs-final-2023112473`。前后端镜像已推送：
...
- `backend-svc` 已绑定共享型 ELB，公网 IP 为 `1.92.115.240`。
- `curl http://1.92.115.240/api/ping` 已验证后端和 Redis 链路正常。
...
- MPI Operator 已部署成功，B-0 MPIJob 已在 CCE 上运行完成，Launcher Job `Complete 1/1`，日志中已保存 π 估算输出。
- 本地 MPI 已完成串并行结果一致性、1/2/4 进程性能测试、Amdahl 图和 4 进程阻塞/非阻塞对比。
- 附加题 C-1 已完成单机 MNIST CNN 与 2 Worker PyTorch DDP 训练对比：单机 `35.509s`，DDP `39.128s`，加速比约 `0.9074`。

后续还需要：

- 建议补 `yjs-mpi:v2` 的 SWR 镜像 Tag 截图
