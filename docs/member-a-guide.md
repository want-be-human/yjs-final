...
- 第二部分方向 B：B-0 环境部署
- 第二部分方向 B：B-1 并行算法实现
- 附加题 1：监控系统中的云资源和 SWR 配合
- 附加题 2：CI/CD 流水线中的 KubeConfig 和部署验证配合
...
- 为监控系统提供 CCE 集群、SWR Region、SWR 组织名和镜像推送权限。
- 协助成员 B 推送监控镜像到 SWR。
- 为 CI/CD 提供 GitHub Secrets 所需的 KubeConfig 和 SWR 登录信息。
- 验证流水线更新 Deployment 后，后端 `/api/ping` 仍然正常。

注意事项：

- KubeConfig、AK、SK、SWR 临时 Token 不能提交到仓库。
- `KUBE_CONFIG` 放到 GitHub Secrets 前需要 base64 编码。
- CI/CD 第一次运行前，要先保证 `backend` 和 `frontend` 两个 Deployment 已经在 CCE 上存在。
